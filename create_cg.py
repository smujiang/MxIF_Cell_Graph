...
for s_id in all_sample_IDs[2:3]: #[0:10]:
    try:
        ...
        print("\t %d nodes in the cell graph (x)" % len(sample_cells_x))
        print("\t %d nodes in the cell graph (y)" % len(sample_cells_y))
        ...
        pos = []
        col_map = []
        for idx, c_x in enumerate(sample_cells_x, start=0):
        ...
        print(f'color map length: {len(col_map)}')
        print(f'pos list length: {len(pos)}')
        cg_creator = CellGraphCreator(cells, self_loop=False)
        cg = cg_creator.graph
        print(f'cg order: {cg.num_nodes()}')
        ...
        cell_graph.remove_edges_from(nx.selfloop_edges(cell_graph))
        plt.figure(1, figsize=[32, 32])
        print(f'cell_graph order: {cell_graph.order()}')
